...
public class RealRunSimulator extends Application {
    ...
    public void playNextAnimation(int index) {
        if (index < pathTList.size()) {
            pathTList.get(index).play();
            index++;
        }
    }

    public void getPathAnimation(Rectangle robot, ArrayList<ArrayList<MoveType>> pathList) {
        //Creating a Path
        //Path path = new Path();
        ArrayList<PathTransition> pathTArrayList = new ArrayList<>();
        ...
        for (int i = 0; i < len; i++) {
            ...
            for (MoveType move : paths) {
                ...
                //seqT.getChildren().add(pathTransition);
                pathTArrayList.add(pathTransition);
            }
            /*
